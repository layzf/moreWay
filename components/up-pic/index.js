...
const upload_file_server = (url, _this, upload_picture_list, j) => {
  const upload_task = wx.uploadFile({
    ...
    success(res) {
      ...
      var imgs = {
        url: upload_picture_list,
        status: _this.data.status == 0 ? 0 : 1
      }
      _this.triggerEvent('upImgData', imgs)
...
const dImage = (e, _this) => {
  ...
  console.log(_this.data.upload_picture_list,"upload_picture_list")
  var imgs={
    url: _this.data.upload_picture_list,
    status: _this.data.status == 0 ? 0 : 1
  }
  _this.triggerEvent('upImgData',imgs )
}
...
const cImage = (_this, count, url) => {
  wx.chooseImage({
    count: _this.data.notli ? count = 6 : _this.data.imgs.length == 0 ? count : count - _this.data.imgs.length,
    sizeType: ['compressed'],
    ...
    success: function(res) {
      ...
      _this.data.notli && count == 6 ? uImage(_this, url) : console.log();
...
const puImage = (e, _this) => {
  console.log(_this.data.imgs, "imgs", e.currentTarget.dataset.idx)
  wx.previewImage({
